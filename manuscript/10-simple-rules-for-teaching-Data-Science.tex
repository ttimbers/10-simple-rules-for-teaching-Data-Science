...
{[}14,15{]}, and Jenny Bryan {[}16{]}, who summed this up nicely in a
...
\section{Rule 4: Give tons and tons of practice and timely
feedback}\label{sec-rule4}

The fourth simple rule is to give tons and tons of practice. Give the
...
repetition leads to learning {[}19{]}. That's not just in humans; it is
more fundamental than that. In the field of animal behavior across the
animal kingdom, repetition is found to lead to learning {[}20,21{]}.
...
(e.g., the swirl R package {[}22{]}), online courses (e.g., Kaggle Learn
{[}23{]}), and popular textbooks (e.g., R for Data Science {[}24{]}).
...
community around them} by Greg Wilson {[}25{]}.
...
programming in computer science courses {[}reviewed in 25{]} and is now
...
including the learnr R package {[}26{]} for R code, and the NBgrader
{[}27{]} and Otter Grader {[}28{]} software packages, which work for
...
{[}29{]} to introduce the students to k-means clustering. Instead of
...
sheet {[}30{]}. In her cheat sheet, she teaches all the various joins
from the dplyr package {[}31{]}. This is a difficult topic for students
...
different regions {[}32{]}. Other examples include the Gapminder
{[}33{]} and UN Votes {[}34{]} data sets. These are particularly nice
...
For example, when we teach the programming language R {[}35{]} using the
suite of tidyverse R packages {[}36{]}. Learners observe that these
...
{[}37{]}, it makes a lot more sense as to why that design choice was
...
chapter in Roger Peng's \emph{R programming data science} book {[}38{]}.
...
boring {[}40{]}. For these reasons, checklists have recently been
...
{[}41{]}. As reviewers for some of these journals and organizations, the
...
Projects give afford students the opportunity to experience the entire
data science workflow. Doing project work is important because it really
...
A timely example of this (when this paper was written) is generative
artificial intelligence (AI). Both chatbots like ChatGPT, but also
agentic code-generation tools such as Claude Code, which can write,
debug, and suggest code for students' assignments. Students are already
using these tools, and perhaps not in the best way(s) possible. For
example, many are using them to take shortcuts on their assignments,
which is not ideal for their learning. However, these tools also offer
opportunities to enhance learning. For instance, they can give students
more timely feedback, which is vital for learning. They also help
students learn to debug code, a key data science skill. We need to
understand these tools' capabilities and use them creatively to benefit
learning. Additionally, these tools are used by industry and academic
data scientists, so it is just as important for our students to learn to
use them effectively as it is to learn to use other data science tools.
The landscape is a lot more complicated, though. First, all the rules
we've outlined in this paper so far can be met with free and/or
open-source tools and resources. However, the use of Generative AI
tools, particularly agentic code-generation tools, often requires a paid
subscription. In a university setting, unless the institution has a site
license, this can create equity issues for students who cannot afford
these tools. In a workshop setting, this would require either learners
to pay for the tool themselves or for the workshop organizers to cover
the cost for all learners, which may not be feasible. Ongoing use of
these tools depends on continued access to payment resources. Second,
there are ethical and privacy issues around the use of Generative AI
tools. Some students may not be willing to engage with them, which is a
choice they should be able to make without fear of negative
consequences. Third, the line between code generated with an AI tool and
plagiarism is blurry, which can create challenges for instructors when
grading assignments and projects. These tools are evolving quickly, and
their long-term impact on data science and education is uncertain. Given
all these complexities, we strongly urge educators to proactively stay
informed about these tools and to directly engage their students in
open, honest discussions about the ethical and practical implications of
using generative AI in their learning and work.
