...
and which we have used and tested in my own data science classroom with success.
...
The first time that we had heard about doing something like this was, there's a wonderful
...
Rule number five is to use tractable or toy data examples. And so what do we mean by this?
...
We think to do is to use a real and rich data set paired with like a real question. But as you're doing
this, make sure that that data set is also accessible. So what do we mean by that is that
...
We might, for example, think that using some deep sequencing data data set might be a great example
for this particular algorithm that we want to teach the students. However, that's because we have this,
...
Number eight is build a safe, inclusive and welcoming community. And we actually think,
...
really be able to know that that's probably what we should spend my time telling them about.
...
them get an idea, get a sense of what the entire workflow is from beginning to end. And as we said,
